...
10. **Retained evidence reviews** embed one complete, strictly validated graph from supported artifact/harness analyzers with a fixed caller-declared, unsigned, advisory disposition. A domain-separated full SHA-256 binds every normalized graph field and receipt metadata. The receipt reader recomputes the graph and receipt identities; checking reuses the saved-graph comparison against a caller-supplied fresh capture. Unknown or version-limited comparisons cannot match. No capture authenticity, human review, installation lineage, restoration history, or runtime authority is inferred, and checking never promotes a baseline.
...
Artifact, harness, and correlation completeness are separate, explicit contracts. Each artifact-closure element and harness field records whether it is observed, declared, user-supplied, mutable, unresolved, endpoint-unverified, unsupported, or unavailable as applicable. The Codex harness snapshot also keeps effective runtime state explicitly unavailable. A complete capability graph supports only its declared-exposure classification; it does not make runtime discovery, initialization, authorization, or effect claims. Retained review eligibility requires complete definitive graph evidence from supported analyzers, but this does not establish trust-decision completeness or a safety rating. Enabled, disabled, and not-declared evidence are equally eligible.
...
The JSON Schemas live at [`schemas/report.schema.json`](../schemas/report.schema.json), [`schemas/comparison.schema.json`](../schemas/comparison.schema.json), [`schemas/signed-report.schema.json`](../schemas/signed-report.schema.json), [`schemas/artifact-snapshot.schema.json`](../schemas/artifact-snapshot.schema.json), [`schemas/capability-graph.schema.json`](../schemas/capability-graph.schema.json), [`schemas/capability-graph-comparison.schema.json`](../schemas/capability-graph-comparison.schema.json), and [`schemas/skill-review-receipt.schema.json`](../schemas/skill-review-receipt.schema.json). The Codex harness uses a separately versioned TypeScript domain and a bounded delta-input reader; a standalone JSON Schema for its deeply nested snapshot and delta documents is not included in v0.7.0. SARIF output targets 2.1.0 and carries scan provenance in its run properties.
