...
Uleravo is one Node.js package with eight explicit boundaries:
...
The public Apache-2.0 core is useful without a hosted service. It consists of the local scanner, Skill and Codex Plugin artifact snapshots, the Codex local-configuration harness, CLI, MCP-focused GitHub Action, report schemas, offline verification, safe fixtures, and public documentation.
...
Artifact and harness completeness are separate, explicit contracts. Each artifact-closure element and harness field records whether it is observed, declared, user-supplied, mutable, unresolved, endpoint-unverified, unsupported, or unavailable as applicable. The Codex harness snapshot also keeps effective runtime state explicitly unavailable. A future advisory decision must not infer trust-decision completeness from the MCP `scan-complete` boolean, artifact `complete` flag, or harness capture flag alone.
...
The JSON Schemas live at [`schemas/report.schema.json`](../schemas/report.schema.json), [`schemas/comparison.schema.json`](../schemas/comparison.schema.json), [`schemas/signed-report.schema.json`](../schemas/signed-report.schema.json), and [`schemas/artifact-snapshot.schema.json`](../schemas/artifact-snapshot.schema.json). The Codex harness uses a separately versioned TypeScript domain and a bounded delta-input reader; a standalone JSON Schema for its deeply nested snapshot and delta documents is not included in v0.7.0. SARIF output targets 2.1.0 and carries scan provenance in its run properties.

## Dependency policy

The production package uses TypeScript for source parsing, YAML for Skill front matter, and smol-toml for inert Codex configuration parsing. Node built-ins provide argument parsing, hashing, filesystem access, URL parsing, and output. Build tooling stays in development dependencies and every dependency is exact-pinned in the lockfile. The GitHub Action bundle vendors the MCP scanner and TypeScript; artifact and harness commands ship in the CLI package but are not exposed as Action inputs.
