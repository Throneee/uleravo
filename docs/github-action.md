...
The reviewed [observation workflow template](../examples/github-actions/uleravo-observe.yml) pins Uleravo and both third-party Actions to full 40-hex SHAs. The current source template pins reviewed 0.8.0-rc.1 candidate commit `27706759b00facf0d42c868b243194ef5a9b933d`. The default `pnpm check` gate requires exactly those three full-SHA references and exactly one `Throneee/uleravo` reference. The candidate publication gate is `node scripts/check-publication-ready.mjs --expected-product-sha 27706759b00facf0d42c868b243194ef5a9b933d`, so an older but syntactically valid Uleravo pin cannot pass. Change `main` under `push.branches` if the consuming repository uses another default branch. The template scans pull requests and default-branch pushes and requests 30-day retention for the JSON and SARIF evidence.

The Action remains MCP-scanner-only. Skill graph, comparison, and retained-review commands are available in the candidate CLI, not as Action inputs. Previously published archives and their bundled templates remain unchanged; use this source template for the current candidate pin.
