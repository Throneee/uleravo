...
The reviewed [observation workflow template](../examples/github-actions/uleravo-observe.yml) pins Uleravo and both third-party Actions to full 40-hex SHAs. The v0.7.0 template pins reviewed product commit `81967c579878fe5dd245bd67a5238e0c5d6ce42f`. The default `pnpm check` gate requires exactly those three full-SHA references and exactly one `Throneee/uleravo` reference. The v0.7.0 release gate is `node scripts/check-publication-ready.mjs --expected-product-sha 81967c579878fe5dd245bd67a5238e0c5d6ce42f`, so an older but syntactically valid Uleravo pin cannot pass. Change `main` under `push.branches` if the consuming repository uses another default branch. The template scans pull requests and default-branch pushes and requests 30-day retention for the JSON and SARIF evidence.
