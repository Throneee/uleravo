...
The reviewed [observation workflow template](../examples/github-actions/uleravo-observe.yml) is staged for the clean public export. It deliberately contains the non-runnable `OWNER/REPOSITORY@PUBLIC_COMMIT_A` placeholder until public Commit A exists. Public Commit B must replace that value with Commit A's full SHA, pass the release smoke test, and pass `pnpm check:publication` before anyone copies the workflow. That final gate requires exactly three full 40-hex Action pins and exactly one `Throneee/uleravo` reference; it intentionally fails on staged Commit A. After the pin is present, change `main` under `push.branches` if the consuming repository uses another default branch. The template scans pull requests and default-branch pushes and requests 30-day retention for the JSON and SARIF evidence.
