...
The repository includes a Node 24 Action that runs the MCP scanner without installing dependencies or executing target code. The current source [observation workflow template](../examples/github-actions/uleravo-observe.yml) pins Uleravo and both third-party Actions to reviewed full 40-hex commit SHAs. Its product pin is reviewed 0.8.0-rc.1 candidate commit `27706759b00facf0d42c868b243194ef5a9b933d`; the immutable v0.7.0 archive retains its earlier template. The default `pnpm check` gate validates the reference shape; the candidate publication gate is `node scripts/check-publication-ready.mjs --expected-product-sha 27706759b00facf0d42c868b243194ef5a9b933d`, so a syntactically valid stale product pin cannot pass. The template disables persisted checkout credentials, scans pull requests and pushes to `main`, and requests 30-day retention for redacted JSON and SARIF evidence. Skill graph and retained-review commands remain CLI-only.
